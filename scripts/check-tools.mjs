...
//
// Навіщо окремі ворота. Скіли `implement` і `review-only` вендорено в ЧОТИРИ теки (по одній на
// інструмент), а рев'юера-субагента задекларовано в П'ЯТИ синтаксисах. Прибери фразу «RED first,
// always» або read-only межу з `.cursor/`, і користувач Cursor дістане мовчки деградований скіл,
// а CI лишиться зеленим: копія, яку ніхто не перевіряє, — це вже не копія, а форк.
//
// Що перевіряємо:
//   1. Фрази-опори скіла `implement` живі в усіх чотирьох теках, а не лише в `.claude/`.
//   2. Чотири копії `review-only` ідентичні й тримають read-only + cited-verdict контракт.
//   3. Рев'юер read-only в усіх п'яти синтаксисах — і, як контроль, імплементер записуваний.
//   4. Кожен `npm run X`, названий у будь-якому `.md`, існує в package.json.
//   5. AGENTS.md називає всі ворота з package.json — ворота, про які агент не прочитає,
//      він не запустить.
//
//   npm run tools:check
...
// `.worktrees/` містить окремі checkout-и зі своїм package.json і своїми воротами. Якщо зайти
// туди з батьківського дерева, markdown однієї гілки перевірятиметься проти scripts іншої.
...
// Production-версія ідеї з `practice/task-3-skill-eval`: воркшоп навмисно починає з одного
// правила й однієї копії, а ці ворота стережуть погоджену трійку в усіх чотирьох теках. Це
// правила ДИСЦИПЛІНИ, без яких TDD вироджується в «напиши код, потім тест, який випадково
// зелений». Формулювання можна міняти — але свідомо, разом із цим списком і копіями скіла.
...
// ── 2. `review-only` однаковий у чотирьох теках ───────────────────────────────────
const REVIEW_SKILL_FILES = [
  '.agents/skills/review-only/SKILL.md', // канонічна Agent Skills копія
  '.claude/skills/review-only/SKILL.md',
  '.cursor/skills/review-only/SKILL.md',
  '.github/skills/review-only/SKILL.md',
];
const reviewSkillTexts = new Map();

for (const file of REVIEW_SKILL_FILES) {
  const text = readOrFail(join(REPO, file), 'копію скіла review-only втрачено');
  if (text !== null) reviewSkillTexts.set(file, text);
}

const reviewCanonical = reviewSkillTexts.get(REVIEW_SKILL_FILES[0]);
if (reviewCanonical !== undefined) {
  const invariants = {
    'Stay read-only.': 'межа забороняє змінювати review-мішень',
    'Cite every finding as `path:line`': 'кожне зауваження має перевірювану адресу',
    'VERDICT: REJECT': 'review завершується машинно видимим вердиктом',
  };

  for (const [phrase, why] of Object.entries(invariants)) {
    v.check(
      reviewCanonical.includes(phrase),
      `${REVIEW_SKILL_FILES[0]}: "${phrase}" — ${why}`,
      `${REVIEW_SKILL_FILES[0]}: ЗНИКЛО "${phrase}" (${why})`
    );
  }

  for (const file of REVIEW_SKILL_FILES.slice(1)) {
    const text = reviewSkillTexts.get(file);
    if (text === undefined) continue;
    v.check(
      text === reviewCanonical,
      `${file}: збігається з ${REVIEW_SKILL_FILES[0]}`,
      `${file}: розійшовся з ${REVIEW_SKILL_FILES[0]} — оновіть усі копії разом`
    );
  }
}

// ── 3. Рев'юер read-only у п'яти синтаксисах ─────────────────────────────────────
...
// 3.1 Claude Code — фронтматер `tools:` як кома-список.
...
// 3.2 Cursor — окремий булів ключ `readonly:`. Ключа `tools:` тут немає взагалі.
...
// 3.3 GitHub Copilot — фронтматер `tools:` як YAML-масив у квадратних дужках, імена в нижньому регістрі.
...
// 3.4 Codex — TOML, межу тримає пісочниця процесу, а не список інструментів.
...
// 3.5 Antigravity — відкритий формат skills-only: ключа інструментів немає ВЗАГАЛІ.
...
// ── 4. Кожен згаданий `npm run X` існує ──────────────────────────────────────────
...
// ── 5. AGENTS.md називає всі ворота з package.json ───────────────────────────────
