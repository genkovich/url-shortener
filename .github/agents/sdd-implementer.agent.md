...
The task brief (`id`, `title`, `acs`, `dod`, `files_hint`) and the red handover from sdd-test-author (test path, run command, quoted failing line). Read the real upstream yourself:
...
- Accepted `adr/` and `sad.md` — the locked decisions and module boundaries. Stay inside this task's `files_hint`; don't edit other modules.
