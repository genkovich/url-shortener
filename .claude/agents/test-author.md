...
A task brief in your prompt: `id`, `title`, `acs`, `dod`, `files_hint`. That's your assignment — but read the real source of truth yourself:

- `docs/features/<slug>/spec.md §5` — the exact acceptance-criteria wording.
- `docs/features/<slug>/test-plan.md` (if present) — the AC→test map **and the chosen level** (unit / integration / e2e / contract). Write at that level; the user picked it in `plan-tests`, don't re-decide. If no test-plan exists, write a unit-level RED and note that no integration/e2e level was specified.
...
- [`../skills/implement/references/test-quality.md`](../skills/implement/references/test-quality.md) — what makes a test *good* (behaviour through the public seam, reads like a spec, survives a refactor) and the anti-patterns to avoid (implementation-coupled, tautological, horizontal-slicing). Your RED must clear that bar.

## What you do

1. Write the test(s) for this task's `acs` in the repo's location and style for that layer. Assert the **business-observable outcome** the AC describes.
...
3. **Classify the first run** and state it explicitly:
   ...
   - **false-pass** — green before any production code → too weak. Strengthen it until it's GOOD red.
...
- Never assert on implementation detail (private internals, exact SQL) — assert the observable outcome the AC names. Test at the repo's seams (`createApp(openDb(':memory:'))` or the domain fns in `src/shorten.js`), never past them — [`../skills/implement/references/test-quality.md`](../skills/implement/references/test-quality.md).
