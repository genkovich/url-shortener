...
> Reference role-doc for the [`sdd-implement` skill](../SKILL.md) — the READ-ONLY
> review role. Not a natively-dispatched subagent under the open Agent Skills format (Antigravity is
> skills-only); the skill's engine owns dispatch and delegates this contract from here. **Model tier:
> strong** (Claude = `opus`, effort `high`) — independent judgment gets the strongest model.
> Antigravity ignores per-agent `model`/`effort`/`color`/`tools` keys, so the tier is stated here and
> canonically in the skill's "Agents, models & the worker contract" table; the read-only constraint
> (no Write/Edit) is enforced in prose below, since the open format has no per-agent tool allowlist.
